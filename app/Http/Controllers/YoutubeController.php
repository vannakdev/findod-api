...
class YoutubeController extends Controller
{
    ...
    public function youtubeVideoUpload(Request $request, $videoName = null)
    {
        ...
        try {
            return [
                'status' => 1,
                'data' => $this->videosInsert($client, $service, $media_file, ['snippet.categoryId' => '22',
                    ...
                    'status.publicStatsViewable' => 'true', ], 'snippet,status', []), ];
        } catch (Exception $ex) {
...
//        $media_file = $request->file('video');
        ...
        try {
            ...
            if ($this->checkVideo($service, $videoId)):
                $this->videosDelete($service, $videoId, ['onBehalfOfContentOwner' => '']);
            endif;
...
//        $media_file = $request->file('video');
        ...
        try {
            ...
            return ['status' =>true, 'data'=> $this->videosDelete($service, $videoId, ['onBehalfOfContentOwner' => ''])];
        } catch (Exception $ex) {
            return ['status' => 0, 'message' => $ex];
        }
    }

    public function checkVideo($service, $id)
    {
        $getVideo = $this->videosListById($service, 'snippet,contentDetails,statistics', ['id' => $id]);
    ...
    public function videosInsert($client, $service, $media_file, $properties, $part, $params)
    {
    ...
    public function videosUpdate($service, $properties, $part, $params)
    {
        $params = array_filter($params);
    ...
    public function videosDelete($service, $id, $params)
    {
        $params = array_filter($params);
